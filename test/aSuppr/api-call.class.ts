...
export class ApiCall {
	 ...
	 * @returns
	 */
	async put<T extends Object>(route: string, query: string | number, body: T) {
		const uri = `${route}/${query}`;
		return await request(this.nestApp.getHttpServer()).put(uri).send(body);
	}
	 ...
	 * @returns
	 */
	async get<T>(route: string, query?: T) {
		const uri = query ? `${route}/${query}` : route;
		return await request(this.nestApp.getHttpServer()).get(uri);
	}
